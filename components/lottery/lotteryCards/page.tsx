...
import { ArrowRight, Ticket, Users, Clock } from "lucide-react";
...
export default function LotteryCards() {
    ...
    const { data: userTicketsData } = useReadContracts({
    ...
    const draws =
        pools?.map((pool, index) => {
            const maxPrizePoolVal = pool.maxPrizePool ?? 0n;
            const poolNumberVal = Number(pool.poolNumber);
            const userTickets = userTicketsData
                ? Number(userTicketsData[index]?.result ?? 0n)
                : 0;
            ...
            return {
                ...
                closes: formatClosesTime(pool.remainingTime, poolNumberVal),
                ticketPrice: `$${Number(formatEther(pool.ticketPrice))}`,
                maxPlayers: Number(pool.maxParticipants),
                round: Number(pool.currentRound),
                open: pool.isOpen,
